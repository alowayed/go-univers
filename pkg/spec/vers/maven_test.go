...
func TestContains_Maven(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "maven exclude - version should be excluded, even in range",
			versRange: "vers:maven/>=1.4.0|!=1.5.0|<=1.6.0",
			version:   "1.5.0",
			want:      false,
			wantErr:   false,
		},
		...
		// Additional edge case tests for != operator in Maven
		{
			name:      "conflicting exact match and exclude constraint",
		...
		{
			name:      "multiple exclude constraints - one matching",
			versRange: "vers:maven/!=1.0.0|!=2.0.0|!=3.0.0",
			version:   "2.0.0",
			want:      false,
			wantErr:   false,
		...
		{
			name:      "multiple exclude constraints - none matching",
			versRange: "vers:maven/!=1.0.0|!=2.0.0|!=3.0.0",
			version:   "4.0.0",
			want:      true,
			wantErr:   false,
		...
		{
			name:      "duplicate exclude constraints should be normalized",
			versRange: "vers:maven/!=1.5.0|!=1.5.0",
			version:   "1.5.0",
			want:      false,
			wantErr:   false,
		},
		{
			name:      "exclude with qualifiers - SNAPSHOT versions",
			versRange: "vers:maven/!=1.0.0-SNAPSHOT",
			version:   "1.0.0-SNAPSHOT",
			want:      false,
			wantErr:   false,
		},
		{
			name:      "exclude with qualifiers - different qualifiers allowed",
			versRange: "vers:maven/!=1.0.0-SNAPSHOT",
